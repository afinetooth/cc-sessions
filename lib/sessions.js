...
const { isAlive } = require('./registry');
...
function collect(projectsDir, registry) {
  ...
  for (const f of walk(projectsDir)) {
    ...
    all.push({
      ...
      // ACTIVE = a Claude process is running for this session right now. A registry entry
      // is only a marker; we probe its pid so an orphaned marker doesn't read as alive.
      pid: reg ? reg.pid : null,
      active: reg ? isAlive(reg.pid) : false,
...
function applyFilters(all, opts) {
  let filtered = all;
  if (opts.activeOnly) filtered = filtered.filter((s) => s.active);
  if (opts.cwdFilter) {
