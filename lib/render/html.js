...
function renderHtml(all, filtered) {
  const rows = filtered
    .map((s) => {
      ...
      // "active" = a Claude process is running for this session right now (probed pid).
      // Plain-text "live" in the cell so the in-browser text filter can match on it.
      const live = s.active
        ? `<span class="live" title="a Claude process is running (pid ${s.pid})">● live</span>`
        : '';
      return `<tr>
      <td>${live}</td>
      <td>${d}</td>
...
<style>
  ...
  .live { display: inline-block; font-size: 11px; font-weight: 600; color: #2e7d32; white-space: nowrap; }
...
<div class="meta">${filtered.length} of ${all.length} total sessions · <strong>${filtered.filter((s) => s.active).length} active</strong> · Generated ${new Date().toLocaleString()}</div>
<input id="search" placeholder="Filter by uuid, cwd, first prompt, or 'live'..." oninput="doFilter()" autofocus>
<table id="t">
<thead><tr>
  <th data-col="0">Status</th>
  <th data-col="1" data-type="date">Modified</th>
  <th data-col="2" data-type="num">KB</th>
  <th data-col="3">UUID</th>
  <th data-col="4">Origin</th>
  <th data-col="5">CWD</th>
  <th data-col="6">First Prompt</th>
  <th></th>
</tr></thead>
<tbody>${rows}</tbody>
</table>
