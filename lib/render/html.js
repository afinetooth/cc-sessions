...
function renderHtml(all, filtered) {
  const rows = filtered
    .map((s) => {
      ...
      const otitle =
        inferred ? 'inferred from transcript markers' :
        s.originSource === 'cache' ? 'remembered from a previous live session' :
        s.originSource === 'path' ? 'detected from working-directory path' :
        s.originSource === 'user-rule' ? 'matched a user rule' :
        s.originSource === 'entrypoint' ? 'from the live process registry' : '';
      const ocls = 'origin origin-' + originSlug(s.origin) + (inferred ? ' inferred' : '');
      return `<tr>
      <td>${d}</td>
      <td class="num">${kb}</td>
      <td><code class="uuid">${escapeHTML(s.uuid)}</code>${sub}</td>
      <td><span class="${ocls}" title="${escapeHTML(otitle)}">${escapeHTML(s.origin)}${inferred ? ' ?' : ''}</span></td>
      <td><code>${escapeHTML(cwd)}</code></td>
      <td class="prompt">${escapeHTML(fp)}</td>
      <td><button onclick="copyCmd(event, '${escapeHTML(s.uuid)}', '${escapeHTML(s.resumeCwd || s.cwd || '')}')">copy resume</button></td>
    </tr>`;
    })
    .join('');
