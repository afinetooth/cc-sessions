...
// s: { entrypoint, cwd, slug }
function detectOrigin(s) {
  for (const r of RULES) if (ruleMatches(r, s)) return r.label;
  const tool = homeToolFromCwd(s.cwd);
  if (tool) return tool;
  if (s.entrypoint && KNOWN_ENTRYPOINTS[s.entrypoint]) return KNOWN_ENTRYPOINTS[s.entrypoint];
  if (s.entrypoint) return prettifyEntrypoint(s.entrypoint);
  return '—';
}
