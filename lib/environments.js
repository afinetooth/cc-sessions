...
//   entrypoint -> live registry entrypoint (perishable; this is what the cache snapshots)
//   cache      -> last-known origin remembered while the session was live (authoritative)
//   inferred   -> transcript IDE markers => "IDE" (a guess; mark it as such in the UI)
//   none       -> "—"
// s: { entrypoint, cwd, slug, uuid, ideMarkers }; cache: { [sessionId]: { origin } }
function detectOrigin(s, cache) {
  ...
  if (s.entrypoint && KNOWN_ENTRYPOINTS[s.entrypoint]) return { origin: KNOWN_ENTRYPOINTS[s.entrypoint], source: 'entrypoint' };
  if (s.entrypoint) return { origin: prettifyEntrypoint(s.entrypoint), source: 'entrypoint' };
  const cached = cache && s.uuid && cache[s.uuid];
  if (cached && cached.origin) return { origin: cached.origin, source: 'cache' };
...
module.exports = { detectOrigin, originSlug, setRules, titleize, prettifyEntrypoint };
